...
public class MemberController {
    ...
    public String registerOwner(@Valid @ModelAttribute("member") Member member, BindingResult bindingResult, Model model) {
        ...
        try {
            if (member.getMbirth() != null) {
                LocalDate.parse(member.getMbirth().toString(), DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            }
        } catch (DateTimeParseException e) {
            bindingResult.rejectValue("mbirth", "error.member", "생년월일이 올바른 형식이 아닙니다. (예: YYYY-MM-DD)");
        }
